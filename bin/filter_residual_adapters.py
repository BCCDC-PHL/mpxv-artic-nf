...
def filter_reads(
    ...
    for R1, R2 in zip(input_R1, input_R2):
        ...
        for f in filter_sequences:
            if contains_adapter(R1.sequence, f, min_match_length) or contains_adapter(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Discard read pairs that contain sequencing adapters that are missed by a trimmer"
    )
    parser.add_argument(
        "--input_R1",
        required=True,
    ...
    filter_reads(
        filter_sequences, min_length, args.input_R1, args.input_R2, output_R1, output_R2
    )
